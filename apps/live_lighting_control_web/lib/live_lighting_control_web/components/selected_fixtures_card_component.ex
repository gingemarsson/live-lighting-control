defmodule LiveLightingControlWeb.SelectedFixturesCardComponent do
  ...
  def render(assigns) do
    ...
    <div class="w-full h-full flex flex-col">
      ...
      <% attributes = @calculated_fixture_values |> Map.values() |> Enum.flat_map(&Map.keys/1) |> Enum.uniq() %>
      ...
      <div class="flex flex-row justify-end mb-2">
        <div
          ...
          phx-value-value={if show_all do "selected" else "all" end}
          phx-value-key={:show}
          phx-value-card-id={@id}
        >
          <p class="text-sm">{if show_all do "Show only selected" else "Show all fixtures" end}</p>
        </div>
      </div>
        ...
        <table class="table-auto w-full border-collapse border border-neutral-600">
          ...
          <tbody>
            <% fixtures = if show_all do @fixtures else LiveLightingControl.Utils.get_selected_fixtures(@fixtures, @selected_fixture_ids) end %>
            <%= for fixture <- fixtures do %>
              ...
              <tr class="hover:bg-neutral-700 text-sm">
                ...
                <%= for attribute_key <- attributes do %>
                  ...
                  <td class={"border border-neutral-600 px-4 py-2 font-monospace #{attribute_source_type_color_class}"}>{attribute_value}</td>
                <% end %>
              </tr>
            <% end %>
          </tbody>
